...
export default function App() {
  ...
  const handleGithubAuth = async () => {
    setAuthError('');
    try {
      // Option A: PocketBase SDK OAuth2 pop-up
      await pb.collection('users').authWithOAuth2({ provider: 'github' });
      setIsAuthOpen(false);
      fetchBookings();
    } catch (err) {
      setAuthError('GitHubi autoriseerimine ebaõnnestus: ' + err.message);
    }
  };
            ...
            <div className="bg-slate-900/90 border border-amber-500/30 rounded-xl p-8 text-center my-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-100 border border-slate-700 font-semibold rounded-lg transition flex items-center justify-center gap-2"
                >
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl max-w-md w-full shadow-2xl relative">
            ...
            <button
              onClick={handleGithubAuth}
              type="button"
              className="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 rounded-lg transition mb-4 flex items-center justify-center gap-3 border border-slate-700 cursor-pointer"
            >
              <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
                <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z" />
              </svg>
            ...
            <div className="relative my-4 flex items-center justify-center">
              ...
              <span className="bg-slate-900 px-3 text-xs text-slate-500 uppercase">või e-postiga</span>
              <div className="border-t border-slate-800 w-full"></div>
            </div>
